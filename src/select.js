...
export default class Select extends Component {
	...
	renderMenu(matchString) {
		...
		return (
			<div className={classPrefix + 'menu-outer'}>
				...
				{matchedOptios.length ?  matchedOptios : (this.state.inputValue.length ? <div className={classPrefix + 'noresults'}>{this.props.noneMatchPrompt}</div> : null)}
				</div>
			</div>
		);
	...
	renderItems() {
		...
		var selectedItems = this.selected.map(function(value, index){
			...
			return (
				<div className={classPrefix + 'item'} key={index} onClick={this.inputFocus.bind(this)}>
					<span className={classPrefix + 'item-icon'} onClick={removeItem.bind(this, 'selected', index)}>×</span>
		...
		var createdItems = this.created.map(function(item, index){
			return (
				<div className={classPrefix + 'item'} key={index} onClick={this.inputFocus.bind(this)}>
					<span className={classPrefix + 'item-icon'} onClick={removeItem.bind(this, 'created', index)}>×</span>
					<span className={classPrefix + 'item-label'}>{this.props.item(item)}</span>
				</div>
			);
		}, this);
...
Select.defaultProps = {
	...
	match: (obj, index, matchStr) => { return (obj.label.indexOf(matchStr) !== -1) || (obj.value.indexOf(matchStr) !== -1);},
	item: (obj) => obj.label,
	isFocused: false,
	isOpen: false,
	searchable: true,
